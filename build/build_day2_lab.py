...
TASKS = [
{
 ...
 "steps":["Attach the report to a new Claude conversation.",
          "Write your own six part prompt for the goal above, then open Claude Design from the same conversation, where the instructor just showed you, and ask it to build the deck. If your account cannot open Claude Design, pair with a neighbour: one drives, one checks the figures.",
...
},
{
 ...
 "steps":["Copy the prompt you wrote in D2.1. The exact same one, not an improved version.",
          "Gamma cannot see the report. Paste the report text under your prompt, then generate and pick a theme. If you want to see what it invents without the source, run it once more with the prompt alone.",
...
},
{
 ...
 "promptSteps":[
  ...
  {"label":"Step 2 &middot; Audit the detail lines",
   "goal":"Get Claude to hunt the detail for anything that would quietly change a total. Use the four checks from the slide: recount, recompute, re-source, re-read. You decide what to look for.",
   "prompt":"Now audit the detail lines for anything that would quietly change a total or a percentage. For each problem you find, give me the row, what is wrong with it, and the effect on the total. Do not stop at the first one.",
...
},
{
 ...
 "promptSteps":[
  {"label":"Step 1 &middot; Ask for the formula, not the answer",
   ...
   "prompt":"Do not give me the number. Give me the Excel formula that calculates the corrected total year to date actual, applying these corrections: [THE CORRECTIONS YOU MADE IN D2.5]. Write it so I can paste it straight into the sheet, and tell me which cells it assumes.",
...
},
{
 ...
 "steps":["Scroll back through the last two days and find the prompts that worked best.",
          ...
          "Then turn your single best prompt into a Skill. Skills are created from Settings, under Capabilities: give it a name, a one line description of when it triggers, and paste the instructions. If your account cannot create Skills, save the same text as the instructions of a new Project. Run it once either way."],
 "promptSteps":[
  ...
  {"label":"Step 2 &middot; Turn the best one into a Skill",
   "goal":"A Skill is a saved instruction Claude loads on its own when the task matches, so you stop pasting the same prompt. Get Claude to write it, then create it in Settings under Capabilities, or as a Project instruction if Skills are off for your account. Then run it.",
